...
// --- Radix Primitives (dark-theme hardcoded) ---
...
>(({ className, sideOffset = 4, showArrow = false, ...props }, ref) => (
  <TooltipPrimitive.Portal>
    <TooltipPrimitive.Content
      ...
      className={cn(
        "z-50 max-w-[280px] rounded-md bg-[#252525] text-white px-2 py-1 text-xs shadow-lg",
        className
      )}
      {...props}
    >
      {props.children}
      {showArrow && (
        <TooltipPrimitive.Arrow className="-my-px fill-[#252525]" />
      )}
    </TooltipPrimitive.Content>
  </TooltipPrimitive.Portal>
));
...
>(({ className, children, ...props }, ref) => (
  <DialogPrimitive.Portal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-1/2 top-1/2 z-50 grid w-full max-w-[90vw] md:max-w-[800px] -translate-x-1/2 -translate-y-1/2 gap-4 border-none bg-transparent p-0 shadow-none",
        className
      )}
      {...props}
    >
      <div className="relative bg-[#303030] rounded-[28px] overflow-hidden shadow-2xl p-1">
        {children}
        <DialogPrimitive.Close className="absolute right-3 top-3 z-10 rounded-full bg-[#303030] p-1 hover:bg-[#515151] transition-all border-none cursor-pointer">
...
const PromptBox = React.forwardRef<HTMLTextAreaElement, PromptBoxProps>(
  ...
  return (
    <div
      className={cn(
        "flex flex-col rounded-[28px] p-2 shadow-sm transition-colors bg-[#303030] border border-transparent cursor-text",
        className
      )}
    >
      <input
      ...
      {imagePreview && (
        <Dialog open={isImageDialogOpen} onOpenChange={setIsImageDialogOpen}>
          <div className="relative mb-1 w-fit rounded-[1rem] px-1 pt-1">
            <button
              type="button"
              className="transition-transform border-none bg-transparent p-0 cursor-pointer"
            ...
            <button
              ...
              className="absolute right-2 top-2 z-10 flex h-4 w-4 items-center justify-center rounded-full bg-[#303030] text-white transition-colors hover:bg-[#515151] border-none cursor-pointer"
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-full text-white transition-colors hover:bg-[#515151] focus-visible:outline-none border-none bg-transparent cursor-pointer"
                >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <PopoverTrigger asChild>
                    <button
                      type="button"
                      className="flex h-8 items-center gap-2 rounded-full p-2 text-sm text-white transition-colors hover:bg-[#515151] focus-visible:outline-none border-none bg-transparent cursor-pointer"
                    >
                ...
                <div className="flex flex-col gap-1">
                  {toolsList.map((tool) => (
                    <button
                      ...
                      className="flex w-full items-center gap-2 rounded-md p-2 text-left text-sm hover:bg-[#515151] border-none bg-transparent text-white cursor-pointer transition-colors"
                    >
